...
class FinalTypeGenerator {
  ...
  private async loadPrismaDMMF(): Promise<void> {
    ...
    try {
      ...
      // Check against expected count (informational only — validateModelCompleteness checks model names)
...
`
  ...
  private getFieldTypeWithJsonMapping(field: any, modelName: string): string {
    ...
    if (kind === 'scalar') {
      ...
      // Relation to another model — property optionality is handled at the property level,
